...
CODE_RESULT ConvertToVectorBits(string input, vector<bool>& output) {
	if (input.size() == 0) {
	...
	for (const auto& c : input) {
		...
		for (int i = 7; i >= 0; --i)
			output.push_back(binary_ascii[i]);
	}

	return OK;
}

CODE_RESULT ConvertToString(vector<bool> input, string& output) {
	if (input.size() == 0) {
		return INVALID_INPUT;
	}
